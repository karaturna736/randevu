...
import { joinWaitlist, updateWaitlist, waitlistSnapshot } from "@/lib/waitlist";
import {
  publicPlan,
  billingSnapshot,
  saveBillingProfile,
  ...
  try {
    ...
    if (p[0] === "public") {
      ...
      return ok({
        ...
        services: await all(
          "SELECT id,name,description,duration,price,color,delivery_mode,active FROM services WHERE tenant_id=? AND active=1",
          b.id,
        ),
        staff: await all(
    ...
    if (p[0] === "admin") {
      ...
      return ok({
        ...
        payments: await all(
          "SELECT id,tenant_id,amount,CASE WHEN state='active' THEN 'paid' ELSE state END status,'subscription' kind,provider,reference provider_ref,transaction_id,user_email,plan,failure_reason,created_at,paid_at,refunded_at FROM onboarding_payments ORDER BY created_at DESC LIMIT 100",
        ),
      });
    }
...
export async function POST(req: Request) {
  try {
    ...
    if (p[0] === "waitlist") {
      ...
      if (id) return ok(await updateWaitlist(id, x));
      return ok(
        await joinWaitlist(await publicBusiness(z.string().parse(x.slug)), x),
        201,
      );
    }
    if (p[0] === "recurring") {
