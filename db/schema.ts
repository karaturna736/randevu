...
export const businesses = sqliteTable("businesses", {
  ...
  terminology: text("terminology").notNull().default("Hizmet"),
  onlineEnabled: integer("online_enabled").notNull().default(0),
...
export const onboardingPayments = sqliteTable(
  "onboarding_payments",
  {
    id: text("id").primaryKey(),
    userId: text("user_id").notNull(),
    ...
    provider: text("provider").notNull().default("iyzico"),
    plan: text("plan").notNull(),
    planReference: text("plan_reference").notNull(),
    amount: integer("amount").notNull(),
    currency: text("currency").notNull().default("TRY"),
...
export const paymentEvents = sqliteTable(
  ...
  (t) => [index("payment_events_payment").on(t.paymentId, t.processedAt)],
);
export const waitlist = sqliteTable(
  ...
  (t) => [
    uniqueIndex("waitlist_request").on(
      t.tenantId,
      t.serviceId,
      t.date,
      t.phone,
    ),
    index("waitlist_open").on(t.tenantId, t.date, t.status),
    foreignKey({
      columns: [t.tenantId, t.serviceId],
      foreignColumns: [services.tenantId, services.id],
    }),
