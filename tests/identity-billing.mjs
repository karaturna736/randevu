...
try{
 ...
 const A=await seedPaidBusiness(db,{id:'neta-a',userId:'owner-a',name:'Test Neta A',slug:'neta-a',paidForMs:5000});
 const B=await seedPaidBusiness(db,{id:'neta-b',userId:'owner-b',name:'Test Neta B',slug:'neta-b',paidForMs:5000});
 check(!!A&&!!B,'Independent paid business fixtures created');
 ...
 const initialSubscription=await db.prepare('SELECT * FROM subscriptions WHERE tenant_id=?').bind(A).first();
 check(initialSubscription&&new Date(initialSubscription.paid_until)-Date.now()<10000,'Starting checkout does not extend the existing paid fixture');
 ...
 const rejectedSubscription=await db.prepare('SELECT * FROM subscriptions WHERE tenant_id=?').bind(A).first();
 check(rejectedSubscription.paid_until===initialSubscription.paid_until,'Rejected callbacks leave subscription unchanged');
