...
const subscription = "subscription_ref_123",
  customer = "customer_ref_123",
  order = "order_ref_123";
const periodStart = new Date().toISOString(),
  periodEnd = new Date(Date.now() + 30 * 86400000).toISOString();
const outboundService = async (request) => {
  const url = new URL(request.url);
  if (!url.hostname.includes("iyzipay.com"))
    throw new Error("Unexpected external call: " + url);
  if (url.pathname === "/v2/subscription/pricing-plans/plan_standard")
    return json({
      status: "success",
      data: {
        price: 990,
  ...
  if (url.pathname === "/v2/subscription/checkoutform/initialize")
    return json({
      status: "success",
      token: "checkout_token_123",
      checkoutFormContent: '<form id="iyzico-checkout"></form>',
    });
  if (url.pathname === "/v2/subscription/checkoutform/checkout_token_123")
    return json({
      status: "success",
      ...
      data: {
        ...
        pricingPlanReferenceCode: "plan_standard",
      },
    });
  if (url.pathname === "/v2/subscription/subscriptions/" + subscription)
    return json({
      status: "success",
      data: {
        referenceCode: subscription,
...
const json = (value) =>
  new TestResponse(JSON.stringify(value), {
    headers: { "content-type": "application/json" },
  });
const secret = "test-secret",
  merchant = "test-merchant";
const bindings = {
  PLATFORM_ADMIN_USER_IDS: "owner",
  ...
  NETA_SELLER_NAME: "Neta",
  NETA_SELLER_ADDRESS: "Test adresi",
  NETA_SUPPORT_EMAIL: "destek@neta.test",
  NETA_SUBSCRIPTION_TERMS_URL: "https://neta.test/kosullar",
  NETA_PLANS_JSON: JSON.stringify([
    {
      code: "normal",
      name: "Standart",
      amount: 99000,
      reference: "plan_standard",
    },
  ]),
};
const mf = new Miniflare({
  modules: files.map((path) => ({
    type: "ESModule",
...
try {
  ...
  const business = {
    name: "Ödeme Test Studio",
    slug: "odeme-test-studio",
  ...
  check(
    rejectedCardConsent.status === 400,
    "Recurring card storage requires explicit provider-side consent",
  );
  const forgedCrossSite = await mf.dispatchFetch(
    "https://neta.test/api/v1/onboarding-payment",
    {
      method: "POST",
      headers: {
        ...headers,
  ...
  check(
    forgedCrossSite.status === 403,
    "Cross-site state-changing requests are rejected before payment logic",
  );
  const idempotencyKey = randomUUID();
  const started = await api("onboarding-payment", {
    ...
    buyer: {
      ...
      city: "İstanbul",
      terms_accepted: true,
      card_storage_accepted: true,
    },
  });
  if (started.status !== 200) console.error("CHECKOUT_RESPONSE", started);
  check(
    started.status === 200 && started.data.form.includes("iyzico-checkout"),
    "Server creates an iyzico checkout using its own plan",
  );
  paymentId = started.data.payment_id;
  check(
    (
      await db
        .prepare("SELECT COUNT(*) n FROM payment_consents WHERE payment_id=?")
        .bind(paymentId)
        .first()
    ).n === 2,
    "Subscription and provider-side card storage approvals are versioned",
  );
  const paymentColumns = (
    await db.prepare("PRAGMA table_info('onboarding_payments')").all()
  ).results.map((row) => row.name);
  check(
    !paymentColumns.some((column) =>
      /card_number|cardholder|cvc|cvv|pan|expire_month|expire_year/i.test(
        String(column),
      ),
    ),
    "Neta payment tables contain no raw card-number, CVC or expiry columns",
  );
  check(
    (
      await api("onboarding-payment", {
        idempotency_key: idempotencyKey,
        business,
        buyer: {
          name: "Test",
    ...
    "Repeated checkout request cannot create a second provider session",
  );
  check(
    !(await db
      .prepare("SELECT 1 ok FROM businesses WHERE slug=?")
      .bind(business.slug)
      .first()),
    "Checkout initialization does not create or activate a business",
  );
  const pending = await api("payment-status");
  check(
    pending.data.account.state === "payment_processing",
    "Account remains payment_processing until provider verification",
  );
  const callback = await mf.dispatchFetch(
    "https://neta.test/api/integrations/iyzico/onboarding-callback",
    {
      method: "POST",
      headers: { "content-type": "application/x-www-form-urlencoded" },
      body: "token=checkout_token_123",
      redirect: "manual",
    },
  );
  check(
    ...
    "Verified iyzico result activates and redirects to panel",
  );
  const activated = await api("payment-status");
  check(
    activated.data.account.state === "active",
    "Only verified payment grants active state",
  );
  check(
    (await api("workspace")).status === 200,
    "Backend workspace opens after activation",
  );
  ...
  check(
    (
      await api("services", {
        ...
        meeting_url: "https://meet.example.test/neta",
        active: 1,
      })
    ).status === 200,
    "Online service stores its protected meeting link",
  );
  ...
  check(
    booked.status === 201 &&
      booked.data.meeting_url === "https://meet.example.test/neta",
  ...
  check(
    waiting.status === 201 &&
      (await api("waitlist?tenant=" + workspace.business.id)).data.requests
        .length === 1,
    "Full-slot customer can join the tenant-scoped waiting list",
  );
  ...
  check(
    duplicate.status === 303 &&
      (
        await db
          .prepare("SELECT COUNT(*) n FROM businesses WHERE slug=?")
          .bind(business.slug)
          .first()
      ).n === 1,
    "Repeated callback does not create a second business",
  );
  const event = {
    iyziEventType: "subscription.order.success",
    subscriptionReferenceCode: subscription,
    orderReferenceCode: "order_ref_renewal",
    customerReferenceCode: customer,
  };
  ...
  const forged = await mf.dispatchFetch(
    "https://neta.test/api/payments/webhook",
    {
      method: "POST",
      headers: {
        "content-type": "application/json",
  ...
  const sendWebhook = () =>
    mf.dispatchFetch("https://neta.test/api/payments/webhook", {
      method: "POST",
      headers: {
        "content-type": "application/json",
  ...
  const first = await sendWebhook(),
    second = await sendWebhook();
  check(
    first.status === 200 &&
      (await first.text()) === "SUCCESS" &&
      second.status === 200 &&
      (await second.text()) === "SUCCESS",
    "Valid duplicate webhooks are acknowledged idempotently",
  );
  check(
    (
      await db
        .prepare("SELECT COUNT(*) n FROM payment_events WHERE event_key=?")
        .bind(
          event.iyziEventType +
            ":" +
            subscription +
    ...
    "Duplicate webhook is recorded exactly once",
  );
