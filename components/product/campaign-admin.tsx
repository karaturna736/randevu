      ...
      <main className="admin-page campaign-page">
        <div className="page-heading campaign-heading">
          <div>
            ...
            <h1>İndirimi büyümeye dönüştürün.</h1>
            <p>
              Kampanyaları hedefleyin, güvenli ödeme önizlemesini yönetin ve
              net geliri tek ekrandan izleyin.
            </p>
          </div>
          ...
          <>
            <div className="stats-grid campaign-stats">
              {([
                ...
                ["Kazanılan işletme", totals.acquired, Users],
            ...
            <section className="panel campaign-list-panel">
              <div className="panel-header">
                <div><h2>Kampanyalar</h2><p className="muted">Silinen kampanyaların finansal kullanım geçmişi korunur.</p></div>
                    ...
                    return (
                      <article className="campaign-card" key={campaign.id}>
                        ...
                        <div className="campaign-value">
                          ...
                          <span>{targetLabels[campaign.target_type]}</span>
                        </div>
            ...
            <section className="panel margin-top campaign-chart-panel">
              ...
              {!daily.length ? <Blank title="Grafik için veri yok" description="Başarılı kampanya ödemeleri günlere göre burada gösterilir." /> : <div className="campaign-chart" role="img" aria-label="Son 14 gün kampanya kullanım grafiği">{daily.map(item=><div className="campaign-chart-day" key={item.day} title={`${item.day}: ${item.uses} kullanım, ${money(item.revenue)} gelir`}><div className="campaign-chart-bar" style={{height:`${Math.max(8,item.uses/maxDaily*100)}%`}}><span>{item.uses}</span></div><small>{new Date(item.day+"T12:00:00Z").toLocaleDateString("tr-TR",{day:"2-digit",month:"2-digit"})}</small></div>)}</div>}
            </section>
            <section className="panel margin-top">
              ...
              {!data.redemptions.length ? <Blank title="Henüz kullanım yok" description="Doğrulanmış kampanya ödemeleri burada görünür." /> : <div className="table-scroll"><Table><TableHeader><TableRow><TableHead>İşletme</TableHead><TableHead>Kampanya</TableHead><TableHead>Normal fiyat</TableHead><TableHead>İndirim</TableHead><TableHead>Ödenen</TableHead><TableHead>Durum / tarih</TableHead></TableRow></TableHeader><TableBody>{data.redemptions.map((item)=><TableRow key={item.id}><TableCell>{item.business_name || "Yeni işletme"}</TableCell><TableCell>{item.campaign_name}<small>{item.campaign_code}</small></TableCell><TableCell>{money(item.original_amount)}</TableCell><TableCell>-{money(item.discount_amount)}</TableCell><TableCell>{money(item.final_amount)}</TableCell><TableCell>{item.status}<small>{new Date(item.created_at).toLocaleString("tr-TR")}</small></TableCell></TableRow>)}</TableBody></Table></div>}
            </section>
          </>
        )}
        <Modal open={!!form} onClose={() => setForm(null)} title={form?.id ? "Kampanyayı düzenle" : "Yeni kampanya"} description="Tutarlar ödeme sağlayıcısına gönderilmeden önce sunucuda tekrar hesaplanır." wide>
          {form && <form className="campaign-form form-stack" onSubmit={save}>
            <div className="form-grid"><Field label="Kampanya adı"><Input required minLength={2} maxLength={120} value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></Field><Field label="Benzersiz kod"><Input required minLength={3} maxLength={32} value={form.code} onChange={e=>setForm({...form,code:e.target.value.toUpperCase().replace(/[^A-Z0-9_-]/g,"")})}/></Field></div>
            ...
            <div className="form-grid"><Field label="İndirim türü"><select className="campaign-select" value={form.discount_type} onChange={e=>setForm({...form,discount_type:e.target.value})}><option value="percentage">Yüzdelik indirim</option><option value="fixed">Sabit tutar indirimi</option></select></Field><Field label={form.discount_type==="percentage"?"İndirim (%)":"İndirim (TL)"}><Input required type="number" min="0.01" max={form.discount_type==="percentage"?"100":"1000000"} step="0.01" value={form.discount_value} onChange={e=>setForm({...form,discount_value:e.target.value})}/></Field></div>
            <div className="form-grid"><Field label="Başlangıç"><Input required type="datetime-local" value={form.starts_at} onChange={e=>setForm({...form,starts_at:e.target.value})}/></Field><Field label="Bitiş"><Input required type="datetime-local" value={form.ends_at} onChange={e=>setForm({...form,ends_at:e.target.value})}/></Field></div>
            ...
            <div className="form-grid"><Field label="Toplam kullanım limiti"><Input type="number" min="1" placeholder="Sınırsız" value={form.total_usage_limit} onChange={e=>setForm({...form,total_usage_limit:e.target.value})}/></Field><Field label="İşletme başına limit"><Input type="number" min="1" placeholder="Sınırsız" value={form.per_business_limit} onChange={e=>setForm({...form,per_business_limit:e.target.value})}/></Field></div>
            <Field label="Hedef türü"><select className="campaign-select" value={form.target_type} onChange={e=>setForm({...form,target_type:e.target.value,business_ids:e.target.value==="selected"?form.business_ids:[]})}>{Object.entries(targetLabels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></Field>
            {form.target_type==="selected"&&<div className="business-picker"><div className="business-picker-tools"><div className="search-box"><Search size={16}/><Input aria-label="İşletme ara" placeholder="İşletme, sahip veya e-posta ara" value={search} onChange={e=>setSearch(e.target.value)}/></div><select className="campaign-select" aria-label="Paket filtresi" value={planFilter} onChange={e=>setPlanFilter(e.target.value)}><option value="all">Tüm paketler</option>{Object.entries(planLabels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></div><div className="business-picker-list">{businesses.map((business)=><label className="business-option" key={business.id}><Checkbox checked={form.business_ids.includes(business.id)} onCheckedChange={checked=>setForm({...form,business_ids:checked?[...form.business_ids,business.id]:form.business_ids.filter((id:string)=>id!==business.id)})}/><span><strong>{business.name}</strong><small>{business.owner_name} · {business.owner_email}</small></span><span><b>{planLabels[business.current_plan]||business.current_plan}</b><small>{business.subscription_status} · {business.branch_count} şube</small></span></label>)}</div><small>{form.business_ids.length} işletme seçildi.</small></div>}
            <div className="campaign-checks vertical"><label className="check-row"><Checkbox checked={form.first_payment_only} onCheckedChange={value=>setForm({...form,first_payment_only:value===true,recurring_enabled:value===true?false:form.recurring_enabled})}/><span>Sadece ilk abonelik ödemesinde geçerli</span></label><label className="check-row"><Checkbox checked={form.recurring_enabled} disabled={form.first_payment_only} onCheckedChange={value=>setForm({...form,recurring_enabled:value===true})}/><span>Aylık yenilemelerde geçerli</span></label><label className="check-row"><Checkbox checked={form.active} onCheckedChange={value=>setForm({...form,active:value===true})}/><span>Kampanyayı aktif kaydet</span></label></div>
