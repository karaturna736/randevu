...
const NAV = [
  ...
  { id: "waitlist", title: "Bekleme listesi", icon: ListPlus },
...
const TITLES: Record<string, [string, string]> = {
  waitlist: [
    "Boşalan saatler değer kazansın.",
    "Uygun saat bekleyen müşterilere tek dokunuşla ulaşın.",
  ],
  receivables: [
...
function SideNavigation({ w, view, navigate, selectBusiness, onSetup }: any) {
      ...
      <SidebarContent>
        ...
        <SidebarMenu>
          {NAV.map((n, i) => (
            <SidebarMenuItem
              key={n.id}
              className={i === 7 ? "nav-section-break" : ""}
            >
              <SidebarMenuButton
              ...
              >
                ...
                <span>
                  {n.id === "services"
                    ? w.business.terminology || n.title
                    : n.title}
                </span>
                {n.id === "appointments" && (
...
export default function Dashboard({
  ...
  const [title, description] = TITLES[view] || TITLES.overview;
  const navTitle =
    view === "services"
      ? w.business.terminology || "Hizmetler"
      : NAV.find((n) => n.id === view)?.title || "Geri kazanma";
  if (
    !loading &&
    w.preview &&
    !loadError &&
    ...
    <SidebarProvider style={{ "--sidebar-width": "238px" } as any}>
      ...
      <SidebarInset className="app-main">
        ...
        <main className="dashboard-content">
          ...
          {view === "waitlist" && <Waitlist key={w.business.id} w={w} />}{" "}
